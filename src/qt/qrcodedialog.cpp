...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MMXVI")
    updateDisplayUnit();
}
...
{
    QString ret = QString("MMXVI://%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MMXVI unit input in lnReqAmount, we generate the URI with MMXVI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MMXVI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
